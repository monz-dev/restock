  ...
  if (error) return (
    <div className="min-h-screen flex items-center justify-center bg-surface p-6">
      <div className="text-center max-w-md">
        ...
        <p className="text-on-surface-ariant mb-4">{error}</p>
        <button onClick={() => router.push('/dashboard')} className="px-4 py-2 bg-primary-container text-on- primary-container rounded">Volver al dashboard</button>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-surface pb-20">
      ...
      {clienteAsignado && (
        <div className="pt-16 px-4 pb-4 bg- surface- container border-b border-outline- variant">
          ...
          {clienteAsignado.direccion && <p className="text-sm text-on-surface-variant">{clienteAsignado.direccion}</p>}
        </div>
      )}
          ...
          <div className="space-y-4">
            {ordenes.map(orden => (
              <div key={orden.orden_id} className="border border-outline- variant rounded-lg overflow-hidden">
                <div className="p-4 bg-surface-high border-b border-outline- variant">
                  <div className="flex justify-between items-center mb-2">
                    <span className={`px-2 py-1 text-xs font-bold rounded uppercase ${
                      orden.estado === 'pendiente' ? 'bg-amber-100 text-amber-800' :
                      orden.estado === 'despachado' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'}`}>
                      {orden.estado}
                    </span>
                  ...
                  {orden.items.map(item => (
                    <div key={item.id} className="flex justify-between items-start">
                      <div className="flex-1">
                        <p className="font-medium text-on-surface">{item.cantidad}x {item.producto_nombre || 'Producto'}</p>
                        <p className="text-sm text-on-surface-variant">{item.unidad_medida}</p>
                      </div>
                      <p className="font-medium text-on-surface">{formatCurrency(Number(item.producto_precio) * item.cantidad)}</p>
                    </div>
                  ))}
                </div>
