...
interface PedidoItem {
  ...
  created_by: string;
  // Joined fields
  producto_ nombre?: string;
  producto_ precio?: number;
  unidad_ medida?: string;
}
    ...
    async function fetchMisPedidos() {
      try {
        ...
        const { data: pedidosData } = await supabase
          .from('pedidos')
          .select('*')
          .eq('cliente_ id', clienteData.cliente.id)
          .order('created_ at', { ascending: false });

        // Get producto details for each pedido
        const allProductoIds = (pedidosData || []).map(p => p.producto_ id);
        const uniqueProductoIds = Array.from(new Set(allProductoIds));

        const { data: productos } = await supabase
          .from('productos')
          .select('id, nombre, precio, unidad_ medida')
        ...
        const pedidosWithDetails = (pedidosData || []).map(p => {
          const producto = productoMap.get(p.producto_ id);
          return {
            ...p,
            producto_ nombre: producto?.nombre,
            producto_ precio: producto?.precio,
            unidad_ medida: producto?.unidad_ medida
          };
        });
  ...
  function groupPedidosByOrden(pedidos: PedidoItem[]): OrdenGroup[] {
    ...
    groups.forEach((items, ordenId) => {
      const total = items.reduce((sum, item) => {
        const precio = Number(item.producto_ precio) || 0;
      ...
      result.push({
        ...
        created_ at: items[0].created_ at,
        total
      });
    });

    // Sort by most recent
    result.sort((a, b) => new Date(b.created_ at).getTime() - new Date(a.created_ at).getTime());
    return result;
  }
      ...
      <main className="pt-4 px-4 max-w-2xl mx- auto">
        {ordenes.length === 0 ? (
          ...
          <div className="space-y-4">
            {ordenes.map(orden => (
              <div key={orden.orden_ id} className="border border-outline-variant rounded-lg overflow-hidden">
                {/* Header */}
                <div className="p-4 bg-surface-high bg-surface-high/50 border-b border-outline-variant">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className="text-sm text-on-surface-variant">{formatDate(orden.created_ at)}</span>
                  </div>
                  <p className="text-sm text-on-surface-variant">Pedido #{orden.orden_ id.slice(0, 8)}</p>
                </div>
                    ...
                    <div key={item.id} className="flex justify-between items-start">
                      <div className="flex-1">
                        <p className="font-medium text-on-surface">
                          {item.cantidad}x {item.producto_ nombre || 'Producto'}
                        </p>
                        <p className="text-sm text-on-surface-variant">{item.unidad_ medida}</p>
                      </div>
                      <p className="font-medium text-on-surface">
                        {formatCurrency(Number(item.producto_ precio) * item.cantidad)}
                      </p>
                    </div>
                  ))}
                </div>
...
export default function MisPedidosPage() {
  return (
    <AuthGuard
      requiredPermiso="ver_pedidos_ propi os"
